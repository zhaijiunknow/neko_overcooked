...
⚠ 而且那张图**根本不是给厨师用的**: `m_nodeMap` 全工程只被**老鼠 NPC** 的
  `GridNavigator` 消费(`GridNavigator.cs:18`), 并且**只在 `Start()` 建一次、永不刷新**
  (`GridNavSpace.cs:44-56`) ⇒ 移动平台的出生格被它**永久记成墙**, 平台后来驶入的格
  却仍是"可走" —— **两个方向都错**。所以只能自己读 `GridManager` 建图(见 `LevelInfo.cs`)。
